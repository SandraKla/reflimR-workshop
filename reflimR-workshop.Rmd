...
A central focus of the course is on mastering the calculation and verification of reference intervals. Participants will explore theoretical concepts as well as practical workflows with *reflimR*. Using the Shiny Application, participants will interact with user-friendly visualizations and interfaces for the process of Reference Interval analysis.
...
The *reflimR* package includes the dataset [*livertests*](https://archive.ics.uci.edu/ml/datasets/HCV+data), which has been utilized for estimating reference intervals using both direct and indirect methods. This dataset contains measurements of eight laboratory biomarkers, recorded in both healthy controls and patients with liver diseases:
...
The algorithm first performs a series of stepwise truncations at increasingly stringent percentile thresholds: 99.5th, 99th, 98.5th, 98th, and finally 97.5th. The rationale is that the distribution of healthy individuals, already truncated to approximate normality, is further refined through this sequential process. Across these five steps, the quantile factor is gradually adjusted toward the target threshold. Afterwards, the iterative truncation continues at the final 97.5th percentile level until no further outliers remain.
...
This is the output of our *reflimR* algorithm for the albumin data in male patients with a refernce interval betwwen 35.1 and 51.1:
...
The package [reflimR](https://cran.r-project.org/web/packages/reflimR/index.html) (≥1.1.0), [DT](https://cran.r-project.org/web/packages/DT/index.html) (≥0.28) and [shinydashboard](https://cran.r-project.org/web/packages/shinydashboard/index.html) (≥ 0.7.2) are downloaded or imported when starting this Shiny Application. The used R-Version must be ≥ 4.3.2.

## 5.2. Dataset

Data from the UC Irvine Machine Learning Repository showing livertests has been preloaded into this Shiny Application (see chapter 4.2.). In addition, the corresponding reference intervals are stored in targetvalues. For new data these columns should be used:
...
On the left, you can use the sidebar to choose the age, parameters and gender. Below in the "Target Values" section, you can load the data from *targetvalues* or enter your own values. In the middle under the "Upload" you can upload new data:
...
Under the tab "*reflimR*" you will see the corresponding plot and the outputs from `reflim()`. By clicking on "Visualization of all plots across every process step", all the plots from the different steps can be displayed:
...
**Exercise 3)** Verify existing Reference Interval values. In the laboratory information system, you already have the following reference interval values for CREA and PROT:
...
**Exercise 5)** Calculate the zlog value for a patient with a measurement of 4 and a Reference Interval of 4.5 to 6.
...
The Reference Interval for CREA is 53-112.7 µmol/L and for PROT 64.7-79.9 mg/L.
...
**Exercise 5)** The *zlog* value for the Reference Interval 4.5 - 6 and the value 4 is -3.565.
